...
MEAT_KEYWORDS = ['鸡', '鸭', '鹅', '猪', '牛', '羊', '鱼', '虾', '蟹', '贝',
                 '鱿', '肉', '排', '翅', '腿', '蹄', '骨', '肚', '肝', '肠',
                 '蛋', '火腿', '培根', '腊', '卤', '熏', '鲍', '蛤', '鳗', '蚝',
                 '海米', '虾米', '虾皮', '金钩', '干贝', '瑶柱']

# 调味料/蛋奶素/植物果肉词：匹配前剔除，避免"鸡精/猪油/蚝油/鸡蛋/牛奶/榴莲肉/肉桂"把素菜误判为荤
SEASONING_PATTERNS = ['鸡精', '味精', '猪油', '蚝油', '生抽', '老抽', '酱油',
                      '食用油', '植物油', '玉米油', '色拉油', '花生油', '调和油',
                      '芝麻油', '香油', '鸡蛋', '鸭蛋', '鹌鹑蛋', '鹅蛋', '蛋液',
                      '蛋黄', '蛋清', '牛奶', '纯牛奶', '鲜牛奶', '淡奶油', '炼乳',
                      '酸奶', '奶酪', '芝士', '黄油', '奶油', '椰奶', '羊奶',
                      '榴莲肉', '大枣肉', '枣肉', '果肉', '椰肉', '瓜肉', '芋肉',
                      '龙眼肉', '桂圆肉', '荔枝肉', '肉桂']
...
def is_meat_dish(recipe: Dict) -> bool:
    ...
    # 剔除调味料/蛋奶素词，避免"鸡精/猪油/蚝油/鸡蛋"把素菜误判为荤
    for p in SEASONING_PATTERNS:
